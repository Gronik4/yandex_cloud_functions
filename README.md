...
3. В Yandex Cloud создать каталог и в нем создать функцию:  
   а). Точка входа: `<Имя сервера>.handler` в нашем случае: `index.handler` - см пункт 1.b  
   в). Загрузить zip-архив и проверить работу перейдя по ссылке. Ответ: `/? не найдено на этом сервере`.  
4. Создать API-шлюз:  
  a). Бессерверные вычисления->API Geteway->Документация->Начало работы с Yandex API Gateway -> Концепции->Расширенные спецификации->Интеграции->Cloud Function. Здесь пример спецификации.  
