...
4. Создать API-шлюз: Бессерверные вычисления->API Geteway.  
  a). Документация->Начало работы с Yandex API Gateway -> Концепции->Расширенные спецификации->Авторизация с помощью функции Cloud Function. Здесь пример спецификации.